...
pub fn calc_current(byte: i16) -> f64 {
    ...
    if formatted.len() > 6 {
        formatted[0..6].parse::<f64>().unwrap_or(float)
    } else {
        float
    }
}
...
pub fn to_serial_number(arr: &[u8], offset: usize, len: usize) -> String {
    let mut sn_arr: Vec<u8> = Vec::with_capacity(len);
    ...
    String::from_utf8(sn_arr).unwrap_or_else(|_| "UTF-8 PARSE ERROR".to_string())
}
